...
// resetState resets the package-level state cgo accumulates during
// a single invocation, so an in-process caller (cmd/cgo/host.Run)
// can reuse the package across calls. The standalone cmd/cgo binary
// runs once per process and never calls this.
func resetState() {
	// util.go
	nerrors = 0
	// main.go
	...
	goarch, goos, gomips, gomips64 = "", "", "", ""
	gccBaseCmd = nil
	// godefs.go
	gofmtBuf.Reset()
	// gcc.go
	tagGen = 0
	typedef = make(map[string]*Type)
	goIdent = make(map[string]*ast.Ident)
	anonymousStructTag = make(map[*dwarf.StructType]string)
	unionWithPointer = make(map[ast.Expr]bool)
	incomplete = "_cgopackage.Incomplete"
	// out.go
	tsanProlog = noTsanProlog
	msanProlog = noMsanProlog

	// Reset only flags that have been set in a previous invocation.
	// VisitAll-then-Set(DefValue) would also fire side-effecting
	// flags like -V (whose Set hook prints version and os.Exits)
	// even when they were never supplied. Visit iterates only flags
	// flag.Parse has actually mutated, which is exactly the set we
	// need to roll back.
...
// Run is cmd/cgo's main entry point, factored out so cmd/cgo/host
// can drive it in-process. The standalone cmd/cgo/main.go just
// calls Run(os.Args[1:]).
//
// args is the argv that the standalone cgo binary would have
// received as os.Args[1:]. Run sets os.Args to mimic the
// standalone shape (cgo plus args) for the duration of flag.Parse
// and any code that reads os.Args (e.g. godefs's comment), then
// restores it on return.
//
// In-process callers must serialise calls and reset cgo's
// package-level state between invocations (cmd/cgo/host.Run does
// both). They should also override ExitFunc with a worker-Goexit
// variant so a fatal cgo error doesn't take down the calling
// process.
// addVersionOnce ensures objabi.AddVersionFlag — which calls
// flag.Var on flag.CommandLine — runs only once across the
// process. Repeated registration panics with "flag redefined";
// the standalone cmd/cgo binary is fine because main() runs
// once, but cmd/cgo/host.Run drives Run multiple times in the
// same process.
...
func Run(rawArgs []string) {
	...
	counter.Open()
	addVersionOnce.Do(objabi.AddVersionFlag) // -V
