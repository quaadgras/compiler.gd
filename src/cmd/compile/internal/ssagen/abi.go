...
func NewSymABIs(gd *base.Invocation) *SymABIs {
	return &SymABIs{
		gd:   gd,
...
func (s *SymABIs) GenABIWrappers() {
	...
	for i, prag := range typecheck.Target(gd).CgoPragmas {
		switch prag[0] {
		case "cgo_export_static", "cgo_export_dynamic":
			symName := s.canonicalize(prag[1])
			pprag := &typecheck.Target(gd).CgoPragmas[i]
	...
	for _, fn := range typecheck.Target(gd).Funcs {
		...
		if hasDefABI {
			if len(fn.Body) != 0 {
				gd.ErrorfAt(fn.Pos(), 0, "%v defined in both Go and assembly", fn)
			}
			fn.ABI = defABI
		}
		...
		if fn.Pragma&ir.CgoUnsafeArgs != 0 {
			...
			// symbol.
			if sym.Linksym(gd).IsLinkname() {
				sym.LinksymABI(gd, fn.ABI).Set(obj.AttrLinkname, true)
			}
			if sym.Linksym(gd).IsLinknameStd() {
				sym.LinksymABI(gd, fn.ABI).Set(obj.AttrLinknameStd, true)
			}
		}
		...
		if len(cgoExport) > 0 && fn.ABIRefs&^obj.ABISetOf(fn.ABI) != 0 {
			gd.Fatalf("cgo exported function %v cannot have ABI wrappers", fn)
		}
		...
		forEachWrapperABI(gd, fn, makeABIWrapper)
	}
}

func forEachWrapperABI(gd *base.Invocation, fn *ir.Func, cb func(gd *base.Invocation, fn *ir.Func, wrapperABI obj.ABI)) {
...
func makeABIWrapper(gd *base.Invocation, f *ir.Func, wrapperABI obj.ABI) {
	if gd.Debug.ABIWrap != 0 {
		fmt.Fprintf(os.Stderr, "=-= %v to %v wrapper for %v\n", wrapperABI, f.ABI, f)
	}

	// Q: is this needed?
	savepos := gd.Pos
	savedcurfn := ir.CurFunc(gd)
	...
	ft := f.Nname.Type()
	if ft.NumRecvs() != 0 {
		gd.ErrorfAt(f.Pos(), 0, "makeABIWrapper support for wrapping methods not implemented")
		return
	}

	// Reuse f's types.Sym to create a new ODCLFUNC/function.
	// TODO(mdempsky): Means we can't set sym.Def in Declfunc, ugh.
	fn := ir.NewFunc(gd, pos, pos, f.Sym(), types.NewSignature(gd, nil,
	...
	if (gd.Ctxt.Arch.Name == "ppc64le" || gd.Ctxt.Arch.Name == "ppc64") && gd.Ctxt.Flag_dynlink {
		// cannot tailcall on PPC64 with dynamic linking, as we need
		// to restore R2 after call.
		tailcall = false
	}
	if gd.Ctxt.Arch.Name == "amd64" && wrapperABI == obj.ABIInternal {
	...
	var tail ir.Node
	call := ir.NewCallExpr(gd, gd.Pos, ir.OCALL, f.Nname, nil)
	call.Args = ir.ParamNames(fn.Type())
	call.IsDDD = fn.Type().IsVariadic()
	tail = call
	if tailcall {
		tail = ir.NewTailCallStmt(gd, gd.Pos, call)
...
func CreateWasmImportWrapper(gd *base.Invocation, fn *ir.Func) bool {
	if fn.WasmImport == nil {
		return false
	}
	if buildcfg.GOARCH != "wasm" {
		gd.FatalfAt(fn.Pos(), "CreateWasmImportWrapper call not supported on %s: func was %v", buildcfg.GOARCH, fn)
	}

	ir.InitLSym(gd, fn, true)

	setupWasmImport(gd, fn)

	pp := objw.NewProgs(gd, fn, 0)
	defer pp.Free(gd)
...
func GenWasmExportWrapper(gd *base.Invocation, wrapped *ir.Func) {
	if wrapped.WasmExport == nil {
		return
	}
	if buildcfg.GOARCH != "wasm" {
		gd.FatalfAt(wrapped.Pos(), "GenWasmExportWrapper call not supported on %s: func was %v", buildcfg.GOARCH, wrapped)
	}
	...
	if ft.ArgWidth() > rtabi.StackSmall {
		gd.ErrorfAt(wrapped.Pos(), 0, "wasmexport function argument too large")
	}
	fn.Pragma |= ir.Nosplit

	ir.InitLSym(gd, fn, true)

	setupWasmExport(gd, fn, wrapped)
	...
	p.To.Sym = gd.Ctxt.Lookup("no_pointers_stackmap")
	pp.Flush(gd)
	// Actual code geneneration is in cmd/internal/obj/wasm.
}

func paramsToWasmFields(gd *base.Invocation, f *ir.Func, pragma string, result *abi.ABIParamResultInfo, abiParams []abi.ABIParamAssignment) []obj.WasmField {
	...
	for _, p := range abiParams {
		...
		if t.IsSIMD() {
			...
			case types.TSTRING:
				// Two parts, (ptr, len)
				wt = obj.WasmPtr
				wfs = append(wfs, obj.WasmField{Type: wt, Offset: p.FrameOffset(gd, result)})
				wfs = append(wfs, obj.WasmField{Type: wt, Offset: p.FrameOffset(gd, result) + int64(types.PtrSize)})
				continue
			case types.TPTR:
				...
				fallthrough
			default:
				gd.ErrorfAt(f.Pos(), 0, "%s: unsupported parameter type %s", pragma, t.String())
			}
		}
		wfs = append(wfs, obj.WasmField{Type: wt, Offset: p.FrameOffset(gd, result)})
	}
...
func resultsToWasmFields(gd *base.Invocation, f *ir.Func, pragma string, result *abi.ABIParamResultInfo, abiParams []abi.ABIParamAssignment) []obj.WasmField {
	if len(abiParams) > 1 {
		gd.ErrorfAt(f.Pos(), 0, "%s: too many return values", pragma)
		return nil
	}
	...
	for i, p := range abiParams {
		...
		if t.IsSIMD() {
			...
			case types.TPTR:
				...
				fallthrough
			default:
				gd.ErrorfAt(f.Pos(), 0, "%s: unsupported result type %s", pragma, t.String())
			}
		}
		wfs[i].Offset = p.FrameOffset(gd, result)
	}
	return wfs
}
...
func setupWasmImport(gd *base.Invocation, f *ir.Func) {
	wi := obj.WasmImport{
...
func setupWasmExport(gd *base.Invocation, f, wrapped *ir.Func) {
	we := obj.WasmExport{
