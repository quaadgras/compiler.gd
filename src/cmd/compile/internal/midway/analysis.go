...
func NewAnalyzer(gd *base.Invocation, pkg *types2.Package, info *types2.Info) *Analyzer {
	return &Analyzer{
		gd:                 gd,
		pkg:                pkg,
		info:               info,
...
func (a *Analyzer) Analyze(files []*syntax.File) bool {
	...
	for {
		...
		if a.gd.Debug.Simd > 0 {
			a.gd.Warn("hasDependentMethod increased from %d to %d", hdmsize, len(a.hasDependentMethod))
		}
		hdmsize = len(a.hasDependentMethod)
		clear(a.visited)
	}
...
func (a *Analyzer) markIfDependent(obj types2.Object) bool {
	...
	if isDep {
		if a.gd.Debug.Simd > 0 {
			a.gd.Warn("%s: %v is simd-dependent", obj.Pos().String(), obj)
		}
		a.isDependentObj[obj] = true
	}
	if isDepMeth {
		if a.gd.Debug.Simd > 0 {
			a.gd.Warn("%s: %v is simd-dependent method", obj.Pos().String(), obj)
		}
