...
func NewDeepCopier(gd *base.Invocation, pkg *types2.Package, info *types2.Info, vecLen int, analyzer *Analyzer, suffix string) *DeepCopier {
	return &DeepCopier{
		gd:       gd,
		VecLen:   vecLen,
		info:     info,
...
func (c *DeepCopier) OnName(id *syntax.Name) *syntax.Name {
	...
	if c.analyzer.isDependentObj[obj] || isBaseSimdTypeObj(obj) {
		...
		if c.gd.Debug.Simd > 0 {
			c.gd.Warn("%s: rewriting name %s to %s", id.Pos().String(), id.Value, newId.Value)
		}
		return newId
	}
	return nil
