...
func NewRewriter(gd *base.Invocation, pkg *types2.Package, info *types2.Info, analyzer *Analyzer, sizes []int) *Rewriter {
	return &Rewriter{
		gd:       gd,
		pkg:      pkg,
		info:     info,
	...
	for _, decl := range fileAST.DeclList {
		...
		case *syntax.FuncDecl:
			...
			if r.analyzer.HasDependentSignature(sig) {
				if r.gd.Debug.Simd > 0 {
					r.gd.Warn("%s: removing body of dependent-sig original function %v", d.Pos().String(), d.Name.Value)
				}
...
func (r *Rewriter) generateForSize(fileAST *syntax.File, k int, newDecls []syntax.Decl) []syntax.Decl {
	copier := NewDeepCopier(r.gd, r.pkg, r.info, k, r.analyzer, fmt.Sprintf("@simd%d", k))
	for _, decl := range fileAST.DeclList {
...
func RewriteWrapper(gd *base.Invocation, pkg *types2.Package, info *types2.Info, files []*syntax.File) bool {
	...
	analyzer := NewAnalyzer(gd, pkg, info)
	if !analyzer.Analyze(files) {
		return false
	}

	CheckPositions(gd, files, "before midway")

	rewriter := NewRewriter(gd, pkg, info, analyzer, sizes)
	rewriter.Rewrite(files)

	CheckPositions(gd, files, "after midway")
