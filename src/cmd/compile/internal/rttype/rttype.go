...
func reflectToType(gd *base.Invocation, rt reflect.Type) *types.Type {
	switch rt.Kind() {
	case reflect.Bool:
	...
	case reflect.Ptr:
		return types.NewPtr(reflectToType(gd, rt.Elem()))
	...
	case reflect.Slice:
		return types.NewSlice(reflectToType(gd, rt.Elem()))
	case reflect.Array:
		return types.NewArray(reflectToType(gd, rt.Elem()), int64(rt.Len()))
	case reflect.Struct:
		fields := make([]*types.Field, rt.NumField())
		for i := 0; i < rt.NumField(); i++ {
			f := rt.Field(i)
			ft := reflectToType(gd, f.Type)
			fields[i] = &types.Field{Sym: &types.Sym{Name: f.Name}, Type: ft}
		}
		return types.NewStruct(fields)
	case reflect.Chan:
		return types.NewChan(reflectToType(gd, rt.Elem()), types.ChanDir(rt.ChanDir()))
	case reflect.String:
		return types.Types[types.TSTRING]
	case reflect.Complex128:
		return types.Types[types.TCOMPLEX128]
	default:
		gd.Fatalf("unhandled kind %s", rt.Kind())
		return nil
	}
}
...
func NewCursor(gd *base.Invocation, lsym *obj.LSym, off int64, t *types.Type) Cursor {
	return Cursor{gd: gd, lsym: lsym, offset: off, typ: t}
}
...
func (c Cursor) WritePtr(target *obj.LSym) {
	if c.typ.Kind() != types.TUNSAFEPTR && c.typ.Kind() != types.TPTR {
		c.gd.Fatalf("can't write ptr, it has kind %s", c.typ.Kind())
	}
	if target == nil {
		objw.Uintptr(c.gd, c.lsym, int(c.offset), 0)
	} else {
		objw.SymPtr(c.gd, c.lsym, int(c.offset), target, 0)
	}
}
func (c Cursor) WritePtrWeak(target *obj.LSym) {
	...
	objw.SymPtrWeak(c.gd, c.lsym, int(c.offset), target, 0)
}
func (c Cursor) WriteUintptr(val uint64) {
	if c.typ.Kind() != types.TUINTPTR {
		c.gd.Fatalf("can't write uintptr, it has kind %s", c.typ.Kind())
	}
	objw.Uintptr(c.gd, c.lsym, int(c.offset), val)
}
func (c Cursor) WriteUint32(val uint32) {
	if c.typ.Kind() != types.TUINT32 {
		c.gd.Fatalf("can't write uint32, it has kind %s", c.typ.Kind())
	}
	objw.Uint32(c.gd, c.lsym, int(c.offset), val)
}
func (c Cursor) WriteUint16(val uint16) {
	if c.typ.Kind() != types.TUINT16 {
		c.gd.Fatalf("can't write uint16, it has kind %s", c.typ.Kind())
	}
	objw.Uint16(c.gd, c.lsym, int(c.offset), val)
}
func (c Cursor) WriteUint8(val uint8) {
	if c.typ.Kind() != types.TUINT8 {
		c.gd.Fatalf("can't write uint8, it has kind %s", c.typ.Kind())
	}
	objw.Uint8(c.gd, c.lsym, int(c.offset), val)
}
func (c Cursor) WriteInt(val int64) {
	if c.typ.Kind() != types.TINT {
		c.gd.Fatalf("can't write int, it has kind %s", c.typ.Kind())
	}
	objw.Uintptr(c.gd, c.lsym, int(c.offset), uint64(val))
}
func (c Cursor) WriteInt32(val int32) {
	if c.typ.Kind() != types.TINT32 {
		c.gd.Fatalf("can't write int32, it has kind %s", c.typ.Kind())
	}
	objw.Uint32(c.gd, c.lsym, int(c.offset), uint32(val))
}
func (c Cursor) WriteBool(val bool) {
	if c.typ.Kind() != types.TBOOL {
		c.gd.Fatalf("can't write bool, it has kind %s", c.typ.Kind())
	}
	objw.Bool(c.gd, c.lsym, int(c.offset), val)
}
...
func (c Cursor) WriteSymPtrOff(target *obj.LSym, weak bool) {
	if c.typ.Kind() != types.TINT32 && c.typ.Kind() != types.TUINT32 {
		c.gd.Fatalf("can't write SymPtr, it has kind %s", c.typ.Kind())
	}
	if target == nil {
		objw.Uint32(c.gd, c.lsym, int(c.offset), 0)
	} else if weak {
		objw.SymPtrWeakOff(c.gd, c.lsym, int(c.offset), target)
	} else {
		objw.SymPtrOff(c.gd, c.lsym, int(c.offset), target)
	}
}
...
func (c Cursor) WriteSlice(target *obj.LSym, off, len, cap int64) {
	if c.typ.Kind() != types.TSLICE {
		c.gd.Fatalf("can't write slice, it has kind %s", c.typ.Kind())
	}
	objw.SymPtr(c.gd, c.lsym, int(c.offset), target, int(off))
	objw.Uintptr(c.gd, c.lsym, int(c.offset)+types.PtrSize, uint64(len))
	objw.Uintptr(c.gd, c.lsym, int(c.offset)+2*types.PtrSize, uint64(cap))
	// TODO: ability to switch len&cap. Maybe not needed here, as every caller
	// passes the same thing for both?
	if len != cap {
		c.gd.Fatalf("len != cap (%d != %d)", len, cap)
	}
}
...
func (c Cursor) Field(name string) Cursor {
	if c.typ.Kind() != types.TSTRUCT {
		c.gd.Fatalf("can't call Field on non-struct %v", c.typ)
	}
	for _, f := range c.typ.Fields() {
		if f.Sym.Name == name {
			return Cursor{gd: c.gd, lsym: c.lsym, offset: c.offset + f.Offset, typ: f.Type}
		}
	}
	c.gd.Fatalf("couldn't find field %s in %v", name, c.typ)
	return Cursor{}
}

func (c Cursor) Elem(i int64) Cursor {
	if c.typ.Kind() != types.TARRAY {
		c.gd.Fatalf("can't call Elem on non-array %v", c.typ)
	}
	if i < 0 || i >= c.typ.NumElem() {
		c.gd.Fatalf("element access out of bounds [%d] in [0:%d]", i, c.typ.NumElem())
	}
	elem := c.typ.Elem()
	return Cursor{gd: c.gd, lsym: c.lsym, offset: c.offset + i*elem.Size(), typ: elem}
}
...
func NewArrayCursor(gd *base.Invocation, lsym *obj.LSym, off int64, t *types.Type, n int) ArrayCursor {
	return ArrayCursor{
		c: NewCursor(gd, lsym, off, t),
		n: n,
	}
}
...
func (a ArrayCursor) Elem(i int) Cursor {
	if i < 0 || i >= a.n {
		a.c.gd.Fatalf("element index %d out of range [0:%d]", i, a.n)
	}
	return Cursor{gd: a.c.gd, lsym: a.c.lsym, offset: a.c.offset + int64(i)*a.c.typ.Size(), typ: a.c.typ}
}
...
func (c Cursor) ModifyArray(n int) (ArrayCursor, int64) {
	if c.typ.Kind() != types.TARRAY {
		c.gd.Fatalf("can't call ModifyArray on non-array %v", c.typ)
	}
	k := c.typ.NumElem()
	return ArrayCursor{c: Cursor{gd: c.gd, lsym: c.lsym, offset: c.offset, typ: c.typ.Elem()}, n: n}, (int64(n) - k) * c.typ.Elem().Size()
}
