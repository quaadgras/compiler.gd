...
func zerorange(gd *base.Invocation, pp *objw.Progs, p *obj.Prog, off, cnt int64, _ *uint32) *obj.Prog {
	...
	for cnt >= 16 && off < 512 {
		p = pp.Append(gd, p, arm64.ASTP, obj.TYPE_REGREG, arm64.REGZERO, arm64.REGZERO, obj.TYPE_MEM, arm64.REGSP, off)
		off += 16
		cnt -= 16
	}
	for cnt != 0 {
		p = pp.Append(gd, p, arm64.AMOVD, obj.TYPE_REG, arm64.REGZERO, 0, obj.TYPE_MEM, arm64.REGSP, off)
		off += 8
		cnt -= 8
	}
	return p
