...
// properly downstream).
func Fixup(gd *base.Invocation) {
	if gd.Flag.Cfg.CoverageInfo == nil {
		return // not using coverage
	}

	metaVarName := gd.Flag.Cfg.CoverageInfo.MetaVar
	pkgIdVarName := gd.Flag.Cfg.CoverageInfo.PkgIdVar
	counterMode := gd.Flag.Cfg.CoverageInfo.CounterMode
	counterGran := gd.Flag.Cfg.CoverageInfo.CounterGranularity
	counterPrefix := gd.Flag.Cfg.CoverageInfo.CounterPrefix
	var metavar *ir.Name
	var pkgidvar *ir.Name

	ckTypSanity := func(nm *ir.Name, tag string) {
		if nm.Type() == nil || nm.Type().HasPointers() {
			gd.Fatalf("unsuitable %s %q mentioned in coveragecfg, improper type '%v'", tag, nm.Sym().Name, nm.Type())
		}
	}

	for _, nm := range typecheck.Target(gd).Externs {
		s := nm.Sym()
		switch s.Name {
		case metaVarName:
			metavar = nm
	...
	if cm == coverage.CtrModeInvalid {
		gd.Fatalf("bad setting %q for covermode in coveragecfg:",
			counterMode)
	}
	...
	default:
		gd.Fatalf("bad setting %q for covergranularity in coveragecfg:",
			counterGran)
	}
	...
	hashv, len := metaHashAndLen(gd)
	if cnames.CounterMode != coverage.CtrModeTestMain {
		registerMeta(gd, cnames, hashv, len)
	}
	if gd.Ctxt.Pkgpath == "main" {
		addInitHookCall(gd, cnames.InitFn, cnames.CounterMode)
	}
}

func metaHashAndLen(gd *base.Invocation) ([16]byte, int) {

	// Read meta-data hash from config entry.
	mhash := gd.Flag.Cfg.CoverageInfo.MetaHash
	if len(mhash) != 32 {
		gd.Fatalf("unexpected: got metahash length %d want 32", len(mhash))
	}
	var hv [16]byte
	for i := 0; i < 16; i++ {
		nib := mhash[i*2 : i*2+2]
		...
		if err != nil {
			gd.Fatalf("metahash bad byte %q", nib)
		}
		hv[i] = byte(x)
	}

	// Return hash and meta-data len
	return hv, gd.Flag.Cfg.CoverageInfo.MetaLen
}

func registerMeta(gd *base.Invocation, cnames names, hashv [16]byte, mdlen int) {
	// Materialize expression for hash (an array literal)
	pos := cnames.InitFn.Pos()
	elist := make([]ir.Node, 0, 16)
	for i := 0; i < 16; i++ {
		elem := ir.NewInt(gd, gd.Pos, int64(hashv[i]))
		elist = append(elist, elem)
	}
	ht := types.NewArray(types.Types[types.TUINT8], 16)
	hashx := ir.NewCompLitExpr(gd, pos, ir.OCOMPLIT, ht, elist)

	// Materalize expression corresponding to address of the meta-data symbol.
	mdax := typecheck.NodAddr(gd, cnames.MetaVar)
	mdauspx := typecheck.ConvNop(gd, mdax, types.Types[types.TUNSAFEPTR])

	// Materialize expression for length.
	lenx := ir.NewInt(gd, gd.Pos, int64(mdlen)) // untyped

	// Generate a call to runtime.addCovMeta, e.g.
	//
	//   pkgIdVar = runtime.addCovMeta(&sym, len, hash, pkgpath, pkid, cmode, cgran)
	//
	fn := typecheck.LookupRuntime(gd, "addCovMeta")
	pkid := coverage.HardCodedPkgID(gd.Ctxt.Pkgpath)
	pkIdNode := ir.NewInt(gd, gd.Pos, int64(pkid))
	cmodeNode := ir.NewInt(gd, gd.Pos, int64(cnames.CounterMode))
	cgranNode := ir.NewInt(gd, gd.Pos, int64(cnames.CounterGran))
	pkPathNode := ir.NewString(gd, gd.Pos, gd.Ctxt.Pkgpath)
	callx := typecheck.Call(gd, pos, fn, []ir.Node{mdauspx, lenx, hashx,
		pkPathNode, pkIdNode, cmodeNode, cgranNode}, false)
	assign := callx
	if pkid == coverage.NotHardCoded {
		assign = typecheck.Stmt(gd, ir.NewAssignStmt(gd, pos, cnames.PkgIdVar, callx))
	}
...
func addInitHookCall(gd *base.Invocation, initfn *ir.Func, cmode coverage.CounterMode) {
	typecheck.InitCoverage(gd)
	pos := initfn.Pos()
	istest := cmode == coverage.CtrModeTestMain
	initf := typecheck.LookupCoverage(gd, "initHook")
	istestNode := ir.NewBool(gd, gd.Pos, istest)
	args := []ir.Node{istestNode}
	callx := typecheck.Call(gd, pos, initf, args, false)
