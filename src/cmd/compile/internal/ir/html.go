...
func (h *HTMLWriterBase) Init(gd *base.Invocation, out io.WriteCloser, reportPath string, doDump func(string) func()) {
	h.gd = gd
...
func NewHTMLWriter(gd *base.Invocation, path string, f *Func, cfgMask string) *HTMLWriter {
	...
	h.Init(gd, out, reportPath, h.FuncHTML)
	h.start()
	return &h
}
...
func (w *HTMLWriterBase) Fatalf(msg string, args ...any) {
	w.gd.FatalfAt(src.NoXPos, msg, args...)
}
...
func (h *HTMLWriter) dumpNodeHeaderHTML(n Node) {
	...
	if h.gd.Debug.DumpPtrs != 0 {
		h.Printf(" p(%p)", n)
	}

	if h.gd.Debug.DumpPtrs != 0 && n.Name() != nil && n.Name().Defn != nil {
		h.Printf(" defn(%p)", n.Name().Defn)
	}

	if h.gd.Debug.DumpPtrs != 0 && n.Name() != nil && n.Name().Curfn != nil {
		h.Printf(" curfn(%p)", n.Name().Curfn)
	}
	if h.gd.Debug.DumpPtrs != 0 && n.Name() != nil && n.Name().Outer != nil {
	...
	if n.Pos().IsKnown() {
		...
		sep := ""
		h.gd.Ctxt.AllPos(n.Pos(), func(pos src.Pos) {
			h.Print(sep)
			sep = " "
