...
func MatchAstDump(gd *base.Invocation, fn *Func, where string) bool {
	if len(gd.Debug.AstDump) == 0 {
		return false
	}
	return matchForDump(gd, fn, gd.Ctxt.Pkgpath, where)
}
...
//
//go:noinline
func matchForDump(gd *base.Invocation, fn *Func, pkgPath, where string) bool {
	return MatchPkgFn(pkgPath, FuncName(fn), gd.Debug.AstDump)
}
...
//
// to os.Stderr.
func AstDump(gd *base.Invocation, fn *Func, why string) {
	err := withLockAndFile(
		gd,
		fn,
		func(w io.Writer) {
...
func withLockAndFile(gd *base.Invocation, fn *Func, dump func(io.Writer)) (err error) {
	...
	astDumpFiles, _ := gd.AstDumpFiles.(map[string]bool)
	if astDumpFiles == nil {
		astDumpFiles = make(map[string]bool)
		gd.AstDumpFiles = astDumpFiles
	}
...
//
// The open writers live on gd (IrHTMLWriters / IrHTMLOrderedFuncs)
// rather than in package globals so concurrent in-process compile
// invocations each keep their own set.
func DumpNodeHTML(gd *base.Invocation, fn *Func, why string, n Node) {
	gd.AstDumpMu.Lock()
	defer gd.AstDumpMu.Unlock()
	htmlWriters, _ := gd.IrHTMLWriters.(map[*Func]*HTMLWriter)
	if htmlWriters == nil {
		htmlWriters = make(map[*Func]*HTMLWriter)
		gd.IrHTMLWriters = htmlWriters
	}
	w, ok := htmlWriters[fn]
	if !ok {
		name := escapedFileName(fn, ".html")
		w = NewHTMLWriter(gd, name, fn, "")
		htmlWriters[fn] = w
		orderedFuncs, _ := gd.IrHTMLOrderedFuncs.([]*Func)
		gd.IrHTMLOrderedFuncs = append(orderedFuncs, fn)
	}
	w.WritePhase(why, why)
}

// CloseHTMLWriters closes every open HTML writer of gd, if any exist.
