...
// gd: the fork's runtime value layouts differ from upstream's 2-word
// string and 2-word interface headers. Equality signatures and the
// generated hash/eq bodies below must use these sizes.
//
//   - string: 3-word header (ptr, hash/bytes, tag|len/bytes); see
//     doc/gd/sso-string.md and types.StringSize.
//   - interface: fat-interface layout {tab/_type, data, inline [16]byte};
//     see types.CalcSize TINTER.
func ifaceSize() int64 {
	return 2*int64(types.PtrSize) + 16
}

func stringSize() int64 {
	return types.StringSize
}

// genhash returns a symbol which is the closure used to compute
// the hash of a value of type t.
func genhash(gd *base.Invocation, t *types.Type) *obj.LSym {
	return genhashSig(gd, eqSignature(gd, t))
}

func genhashSig(gd *base.Invocation, sig string) *obj.LSym {
	if len(sig) > 0 && sig[0] == sigAlign {
		_, sig = parseNum(gd, sig[1:])
	}
	switch sig {
	case "":
	...
	closure := TypeLinksymLookup(gd, ".hashfunc."+sig)
	if len(closure.P) > 0 { // already generated
		return closure
	}

	if sig[0] == sigMemory {
		n, rest := parseNum(gd, sig[1:])
		if rest == "" {
			...
			ot = objw.Uintptr(gd, closure, ot, uint64(n)) // size encoded in closue
...
func hashFunc(gd *base.Invocation, sig string) *ir.Func {
	sym := types.TypeSymLookup(gd, ".hash."+sig)
	if sym.Def != nil {
	...
	fn := ir.NewFunc(gd, pos, pos, sym, types.NewSignature(gd, nil,
		[]*types.Field{
			types.NewField(pos, typecheck.Lookup(gd, "p"), types.Types[types.TUNSAFEPTR]),
	...
	ptr := func() ir.Node {
		c := ir.NewBasicLit(gd, pos, types.Types[types.TUINTPTR], constant.MakeInt64(off))
		return ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, np, c)
	}
	// hash data using function name at np+off.
	// Increment off by size.
	hash := func(name string, size int64) {
		p := ptr()
		hashFn := typecheck.LookupRuntime(gd, name)
		call := ir.NewCallExpr(gd, pos, ir.OCALL, hashFn, []ir.Node{p, nh})
		fn.Body.Append(ir.NewAssignStmt(gd, pos, nh, call))
		off += size
	}
	...
	for len(sig) > 0 {
		...
		case sigMemory:
			var n int64
			n, sig = parseNum(gd, sig)
			switch {
			case n == 4:
				p := ptr()
				memhash := typecheck.LookupRuntime(gd, "memhash32")
				call := ir.NewCallExpr(gd, pos, ir.OCALL, memhash, []ir.Node{p, nh})
				fn.Body.Append(ir.NewAssignStmt(gd, pos, nh, call))
			case n == 8:
				p := ptr()
				memhash := typecheck.LookupRuntime(gd, "memhash64")
				call := ir.NewCallExpr(gd, pos, ir.OCALL, memhash, []ir.Node{p, nh})
				fn.Body.Append(ir.NewAssignStmt(gd, pos, nh, call))
			default:
				p := ptr()
				memhash := typecheck.LookupRuntime(gd, "memhash")
				size := ir.NewBasicLit(gd, pos, types.Types[types.TUINTPTR], constant.MakeInt64(n))
				call := ir.NewCallExpr(gd, pos, ir.OCALL, memhash, []ir.Node{p, nh, size})
				fn.Body.Append(ir.NewAssignStmt(gd, pos, nh, call))
			}
			off += n
		case sigFloat32:
		...
		case sigFloat64:
			hash("f64hash", 8)
		case sigString:
			hash("strhash", stringSize())
		case sigEface:
			hash("nilinterhash", ifaceSize())
		case sigIface:
			hash("interhash", ifaceSize())
		case sigSkip:
			var n int64
			n, sig = parseNum(gd, sig)
			off += n
		case sigArrayStart:
			var n int64
			var elemSig string
			n, elemSig, sig = parseArray(gd, sig)
			elemSize := sigSize(gd, elemSig)

			// Loop N times, calling hash function for the element.
			//     for i := off; i < off + N*elemSize; i += elemSize {
			//         h = elemfn(p+i, h)
			//     }
			elemFn := hashFunc(gd, elemSig).Nname
			idx := typecheck.TempAt(gd, pos, ir.CurFunc(gd), types.Types[types.TUINTPTR])
			init := ir.NewAssignStmt(gd, pos, idx, ir.NewInt(gd, pos, off))
			cond := ir.NewBinaryExpr(gd, pos, ir.OLT, idx, ir.NewInt(gd, pos, off+n*elemSize))
			post := ir.NewAssignStmt(gd, pos, idx, ir.NewBinaryExpr(gd, pos, ir.OADD, idx, ir.NewInt(gd, pos, elemSize)))

			p := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, np, idx)
			call := typecheck.Call(gd, pos, elemFn, []ir.Node{p, nh}, false)
			as := ir.NewAssignStmt(gd, pos, nh, call)
			loop := ir.NewForStmt(gd, pos, init, cond, post, []ir.Node{as}, false)
			fn.Body.Append(loop)
			off += n * elemSize
		}
	}

	fn.Body.Append(ir.NewReturnStmt(gd, pos, []ir.Node{nh}))
...
func geneqSig(gd *base.Invocation, sig string) *obj.LSym {
	...
	closure := TypeLinksymLookup(gd, ".eqfunc."+sig)
	if len(closure.P) > 0 { // already generated
		return closure
	}
	...
	if sig[0] == sigMemory {
		...
		if rest == "" {
			...
			ot = objw.Uintptr(gd, closure, ot, uint64(n))
			objw.Global(gd, closure, int32(ot), obj.DUPOK|obj.RODATA)
			return closure
		}
	}
...
func eqFunc(gd *base.Invocation, sig string) *ir.Func {
	sym := types.TypeSymLookup(gd, ".eq."+sig)
	if sym.Def != nil {
		return sym.Def.(*ir.Name).Func
	}
	...
	fn := ir.NewFunc(gd, pos, pos, sym, types.NewSignature(gd, nil,
		[]*types.Field{
			types.NewField(pos, typecheck.Lookup(gd, "p"), types.Types[types.TUNSAFEPTR]),
			types.NewField(pos, typecheck.Lookup(gd, "q"), types.Types[types.TUNSAFEPTR]),
		},
		[]*types.Field{
	...
	if len(sig) > 0 && sig[0] == sigAlign {
		sig = sig[1:]
		align, sig = parseNum(gd, sig)
	}
	unalignedOk := gd.Ctxt.Arch.CanMergeLoads
	...
	test := func(eq ir.Node) {
		...
		if lastTest != nil {
			nif := ir.NewIfStmt(gd, pos, lastTest, nil, []ir.Node{ir.NewBranchStmt(gd, pos, ir.OGOTO, neq)})
			fn.Body.Append(nif)
		}
		lastTest = eq
	}
	// loadAt loads data of type t from np+at and nq+at.
	// It does not change off.
	loadAt := func(t *types.Type, at int64) (ir.Node, ir.Node) {
		c := ir.NewBasicLit(gd, pos, types.Types[types.TUINTPTR], constant.MakeInt64(at))
		p := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, np, c)
		q := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, nq, c)
		x := ir.NewStarExpr(gd, pos, ir.NewConvExpr(gd, pos, ir.OCONVNOP, t.PtrTo(), p))
		y := ir.NewStarExpr(gd, pos, ir.NewConvExpr(gd, pos, ir.OCONVNOP, t.PtrTo(), q))
		return x, y
	}
	...
	// cmp compares x and y and jumps to neq if they are not equal.
	cmp := func(x, y ir.Node) {
		test(ir.NewBinaryExpr(gd, pos, ir.OEQ, x, y))
	}
	...
	//
	// gd: strings are 3-word SSO headers whose first word is not
	// necessarily a pointer and whose length word carries tag bits, so
	// we never load the header words raw. Instead we load the value as
	// a string and go through compare.EqString, which lowers OSPTR/OLEN
	// with full knowledge of the inline/heap representation.
	var pendingStrings []int64
	flushStrings := func() {
		for _, x := range pendingStrings {
			sx, sy := loadAt(types.Types[types.TSTRING], x)
			sx, sy = typecheck.Expr(gd, sx), typecheck.Expr(gd, sy)
			// Note: we already checked that the lengths are equal.
			_, eqmem := compare.EqString(gd, sx, sy)
			test(eqmem)
			hasCall = true
		}
	...
	for len(sig) > 0 {
		...
		case sigMemory:
			...
			if n > 64 { // TODO: why 64?
				...
				p := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, np, c)
				q := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, nq, c)
				len := ir.NewBasicLit(gd, pos, types.Types[types.TUINTPTR], constant.MakeInt64(n))
				memeq := typecheck.LookupRuntime(gd, "memequal")
				test(typecheck.Call(gd, pos, memeq, []ir.Node{p, q, len}, false))
				hasCall = true
				off += n
				n = 0
			}
			...
			for n != 0 {
				...
				case n >= 8 && (unalignedOk || align >= 8 && off%8 == 0):
					cmp(load(types.Types[types.TUINT64]))
					n -= 8
				...
				case n >= 4 && (unalignedOk || align >= 4 && off%4 == 0):
					cmp(load(types.Types[types.TUINT32]))
					n -= 4
				...
				case n >= 2 && (unalignedOk || align >= 2 && off%2 == 0):
					cmp(load(types.Types[types.TUINT16]))
					n -= 2
				default:
					cmp(load(types.Types[types.TUINT8]))
					n--
				}
			}
		case sigFloat32:
			cmp(load(types.Types[types.TFLOAT32]))
		case sigFloat64:
			cmp(load(types.Types[types.TFLOAT64]))
		case sigString:
			// Compare just the lengths right now.
			// Save the contents for later.
			pendingStrings = append(pendingStrings, off)
			sx, sy := load(types.Types[types.TSTRING])
			sx, sy = typecheck.Expr(gd, sx), typecheck.Expr(gd, sy)
			eqlen, _ := compare.EqString(gd, sx, sy)
			test(eqlen)
		case sigEface, sigIface:
			...
			flushStrings()
			// gd fat-iface: compare the tab/type words, then hand the
			// runtime pointers to the full interface headers so it can
			// dispatch on the concrete type's storage mode (direct /
			// inline / spread / boxed). See compare.EqInterface.
			//   func ifaceeq(tab *uintptr, x, y unsafe.Pointer) bool
			//   func efaceeq(typ *uintptr, x, y unsafe.Pointer) bool
			start := off
			typeX, typeY := load(types.Types[types.TUINTPTR].PtrTo())
			cmp(typeX, typeY)
			c := ir.NewBasicLit(gd, pos, types.Types[types.TUINTPTR], constant.MakeInt64(start))
			dataX := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, np, c)
			dataY := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, nq, c)
			off = start + ifaceSize()
			var eqFn *ir.Name
			if kind == sigEface {
				eqFn = typecheck.LookupRuntime(gd, "efaceeq")
			} else {
				eqFn = typecheck.LookupRuntime(gd, "ifaceeq")
			}
			test(typecheck.Call(gd, pos, eqFn, []ir.Node{typeX, dataX, dataY}, false))
			hasCall = true
		case sigSkip:
			var n int64
			n, sig = parseNum(gd, sig)
		...
		case sigArrayStart:
			...
			//     }
			elemFn := eqFunc(gd, sigTrimSkip(elemSig)).Nname
			idx := typecheck.TempAt(gd, pos, ir.CurFunc(gd), types.Types[types.TUINTPTR])
			init := ir.NewAssignStmt(gd, pos, idx, ir.NewInt(gd, pos, off))
			cond := ir.NewBinaryExpr(gd, pos, ir.OLT, idx, ir.NewInt(gd, pos, off+n*elemSize))
			post := ir.NewAssignStmt(gd, pos, idx, ir.NewBinaryExpr(gd, pos, ir.OADD, idx, ir.NewInt(gd, pos, elemSize)))
			...
			p := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, np, idx)
			q := ir.NewBinaryExpr(gd, pos, ir.OUNSAFEADD, nq, idx)
			call := typecheck.Call(gd, pos, elemFn, []ir.Node{p, q}, false)
			nif := ir.NewIfStmt(gd, pos, call, nil, []ir.Node{ir.NewBranchStmt(gd, pos, ir.OGOTO, neq)})
			loop := ir.NewForStmt(gd, pos, init, cond, post, []ir.Node{nif}, false)
			fn.Body.Append(loop)
			off += n * elemSize
	...
	flushStrings()
	if lastTest == nil {
		lastTest = ir.NewBool(gd, pos, true)
	}
	as := ir.NewAssignStmt(gd, pos, nr, lastTest)
	fn.Body.Append(as)

	// ret:
	//   return
	ret := typecheck.AutoLabel(gd, ".ret")
	fn.Body.Append(ir.NewLabelStmt(gd, pos, ret))
	fn.Body.Append(ir.NewReturnStmt(gd, pos, nil))

	// neq:
	//   r = false
	//   return (or goto ret)
	fn.Body.Append(ir.NewLabelStmt(gd, pos, neq))
	fn.Body.Append(ir.NewAssignStmt(gd, pos, nr, ir.NewBool(gd, pos, false)))
	if hasCall {
		// Epilogue is large, so share it with the equal case.
		fn.Body.Append(ir.NewBranchStmt(gd, pos, ir.OGOTO, ret))
	} else {
...
func EqFor(gd *base.Invocation, t *types.Type) (ir.Node, bool) {
	switch types.AlgType(gd, t) {
	case types.AMEM:
		return typecheck.LookupRuntime(gd, "memequal"), true
	case types.ASPECIAL:
		fn := eqFunc(gd, eqSignature(gd, t))
...
func eqSignature(gd *base.Invocation, t *types.Type) string {
	e := eqSigBuilder{gd: gd}
	if !gd.Ctxt.Arch.CanMergeLoads { // alignment only matters if we can't use unaligned loads
...
func (e *eqSigBuilder) build(t *types.Type) {
	...
	case types.TARRAY:
		if types.AlgType(e.gd, t) == types.AMEM {
	...
	default:
		e.gd.Fatalf("eqSigBuilder %v", t)
	}
}

// Parse and remove the number at the start of s.
func parseNum(gd *base.Invocation, s string) (int64, string) {
	n := 0
...
func parseArray(gd *base.Invocation, sig string) (int64, string, string) {
	var n int64
	n, sig = parseNum(gd, sig)
	...
	for {
		if i == len(sig) {
			gd.Fatalf("mismatched brackets in %s", sig)
		}
		switch sig[i] {
		case sigArrayStart:
			depth++
...
func sigSize(gd *base.Invocation, sig string) int64 {
	var size int64
	for len(sig) > 0 {
		kind := sig[0]
		sig = sig[1:]
		...
		case sigString:
			size += stringSize()
		case sigIface, sigEface:
			size += ifaceSize()
		case sigArrayStart:
			var n int64
			var elemSig string
			n, elemSig, sig = parseArray(gd, sig)
			size += n * sigSize(gd, elemSig)
		}
	}
	return size
}
