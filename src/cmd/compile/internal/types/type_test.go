...
func TestFormatInterfaceUnexportedMethodUsesGoModeForSignature(t *testing.T) {
	gd := testingInvocation
	oldLocal := gd.LocalPkg
	oldPtr, oldReg, oldMax := PtrSize, RegSize, MaxWidth
	t.Cleanup(func() {
		gd.LocalPkg = oldLocal
		PtrSize, RegSize, MaxWidth = oldPtr, oldReg, oldMax
	})
	gd.LocalPkg = NewPkg(gd, "main", "main")
	PtrSize = 8
	RegSize = 8
	MaxWidth = 1 << 50

	lib := NewPkg(gd, "example.com/lib", "lib")
	...
	sig := NewSignature(gd, nil, nil, []*Field{NewField(src.NoXPos, nil, namedT)})
	iface := NewInterface([]*Field{NewField(src.NoXPos, LocalPkg(gd).Lookup("unexported"), sig)})
	CalcSize(gd, iface) // populate interface method set for printing (expandiface)
