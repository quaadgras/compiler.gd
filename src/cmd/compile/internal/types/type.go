...
func NewArray(elem *Type, bound int64) *Type {
	if bound < 0 {
		fatal.Error("NewArray: invalid bound %v", bound)
	}
	t := newType(TARRAY)
...
func NewSlice(elem *Type) *Type {
	if t := elem.cache.slice.Load(); t != nil {
		...
		if elem.HasShape() != t.HasShape() {
			fatal.Error("Incorrect HasShape flag for cached slice type")
		}
		return t
	}
...
func NewPtr(elem *Type) *Type {
	...
	if t := elem.cache.ptr.Load(); t != nil {
		...
		if elem.HasShape() != t.HasShape() {
			fatal.Error("Incorrect HasShape flag for cached pointer type")
		}
		return t
	}
...
func SubstAny(t *Type, types *[]*Type) *Type {
	...
	case TANY:
		if len(*types) == 0 {
			fatal.Error("SubstArgTypes: not enough argument types")
		}
		t = (*types)[0]
		*types = (*types)[1:]
...
func (t *Type) copy() *Type {
	...
	case TTUPLE, TSSA, TRESULTS:
		fatal.Error("ssa types cannot be copied")
	}
...
func (t *Type) wantEtype(et Kind) {
	if t.kind != et {
		fatal.Error("want %v, but have %v", et, t)
	}
}
...
func (t *Type) Elem() *Type {
	...
	fatal.Error("Type.Elem %s", t.kind)
	return nil
}
...
func (t *Type) setFields(fields []*Field) {
	...
	if t.widthCalculated() {
		fatal.Error("SetFields of %v: width previously calculated", t)
	}
	t.wantEtype(TSTRUCT)
	t.fields().Set(fields)
}
...
func (t *Type) OffsetOf(name string) int64 {
	if t.kind != TSTRUCT {
		fatal.Error("can't call OffsetOf on non-struct %v", t)
	}
	...
	fatal.Error("couldn't find field %s in %v", name, t)
	return -1
}
...
func (t *Type) NumComponents(countBlank componentsIncludeBlankFields) int64 {
	...
	case TSTRUCT:
		if t.IsFuncArgStruct() {
			fatal.Error("NumComponents func arg struct")
		}
		var n int64
		for _, f := range t.Fields() {
...
func (t *Type) SoleComponent() *Type {
	...
	case TSTRUCT:
		if t.IsFuncArgStruct() {
			fatal.Error("SoleComponent func arg struct")
		}
		if t.NumFields() != 1 {
			return nil
		}
...
func (t *Type) SetUnderlying(underlying *Type) {
	...
	if ft.Embedlineno.IsKnown() {
		if t.IsPtr() || t.IsUnsafePtr() {
			fatal.ErrorAt(ft.Embedlineno, errors.InvalidPtrEmbed, "embedded type cannot be a pointer")
		}
	}
}
...
func NewSignature(gd *base.Invocation, recv *Field, params, results []*Field) *Type {
	...
	if needsRewrite {
		outBufs := BuildOutBufFields(gd, results)
...
func IsReflexive(t *Type) bool {
	...
	default:
		fatal.Error("bad type for map key: %v", t)
		return false
	}
}
...
func FloatForComplex(t *Type) *Type {
	...
	fatal.Error("unexpected type: %v", t)
	return nil
}
