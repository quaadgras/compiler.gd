...
func newPkgWriter(gd *base.Invocation, m posMap, pkg *types2.Package, info *types2.Info, otherInfo map[*syntax.FuncLit]bool) *pkgWriter {
	return &pkgWriter{
		PkgEncoder: pkgbits.NewPkgEncoder(uirVersion, gd.Debug.SyncFrames),
		gd:         gd,
...
//
// Universal extension (Phase G.2.1): no CompilingRuntime gate, no
...
func (pw *pkgWriter) objIdx(obj types2.Object) index {
	...
	if isDefinedType(obj) && obj.Pkg() == pw.curpkg {
		...
		if !ok {
			pw.gd.Fatalf("%v not in pw.typDecls", obj.(*types2.TypeName))
		}
...
func (w *writer) doObj(wext *writer, obj types2.Object) pkgbits.CodeObj {
	...
	case *types2.Func:
		if w.gd.Flag.LowerH > 0 {
			// Unified IR panics are the worst; this is a huge help in debugging them.
			defer func() {
				if p := recover(); p != nil {
					w.gd.Logf("Intercepted unified IR writer panic for function %s, repanicking", obj.FullName())
					panic(p)
				}
			}()
		}
	...
	case *types2.TypeName:
		...
		if len(gmethods) > 0 {
			assert(w.gd, w.Version().Has(pkgbits.GenericMethods))
		}
...
func (w *writer) methodExpr(expr *syntax.SelectorExpr, recv types2.Type, sel *types2.Selection) {
	...
	if isConcreteMethod(w.gd, sig) {
		tname, tExplicits := splitNamed(w.gd, types2.Unalias(deref2(recv)).(*types2.Named))
		var info objInfo
		if isGenericMethod(sig) {
...
func isConcreteMethod(gd *base.Invocation, typ types2.Type) bool {
	sig, ok := typ.(*types2.Signature)
	return ok && sig.Recv() != nil && !isInterface(gd, sig.Recv().Type())
}
...
func splitNamed(gd *base.Invocation, typ *types2.Named) (*types2.TypeName, []types2.Type) {
...
func splitAlias(gd *base.Invocation, typ *types2.Alias) (*types2.TypeName, []types2.Type) {
	orig := typ.Origin()
