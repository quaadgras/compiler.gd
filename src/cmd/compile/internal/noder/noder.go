...
func LoadPackage(gd *base.Invocation, filenames []string) {
	gd.Timer.Start("fe", "parse")
	...
	go func() {
		for i, filename := range filenames {
			...
			go func() {
				...
				p.file, _ = syntax.Parse(gd, fbase, f, p.error, p.pragma, syntax.CheckBranches) // errors are tracked via p.error
			}()
		}
	}()
	...
	for _, p := range noders {
		for e := range p.err {
			gd.ErrorfAt(m.makeXPos(gd, e.Pos), 0, "%s", e.Msg)
		}
		if p.file == nil {
			gd.ErrorExit()
		}
	...
	gd.Timer.AddEvent(int64(lines), "lines")
...
// returned unchanged.
func trimFilename(gd *base.Invocation, b *syntax.PosBase) string {
	filename := b.Filename()
	if !b.Trimmed() {
		dir := ""
		if b.IsFileBase() {
			dir = gd.Ctxt.Pathname
		}
		filename = objabi.AbsFile(dir, filename, gd.Flag.TrimPath)
	}
	return filename
}
...
func (p *noder) pragma(gd *base.Invocation, pos syntax.Pos, blankLine bool, text string, old syntax.Pragma) syntax.Pragma {
	...
	case strings.HasPrefix(text, "go:linkname "):
		...
		if len(f) == 3 {
			target = f[2]
		} else if gd.Ctxt.Pkgpath != "" {
			// Use the default object symbol name if the
			// user didn't provide one.
			target = objabi.PathToPrefix(gd.Ctxt.Pkgpath) + "." + f[1]
		} else {
	...
	case strings.HasPrefix(text, "go:cgo_import_dynamic "):
		...
		if len(fields) >= 4 {
			...
			if lib != "" && !safeArg(lib) && !isCgoGeneratedFile(gd, pos) {
	...
	case strings.HasPrefix(text, "go:cgo_"):
		...
		if !isCgoGeneratedFile(gd, pos) && !gd.Flag.Std {
	...
	default:
		...
		if !gd.Flag.CompilingRuntime && flag&runtimePragmas != 0 {
			p.error(syntax.Error{Pos: pos, Msg: fmt.Sprintf("//%s only allowed in runtime", verb)})
		}
		if flag == ir.UintptrKeepAlive && !gd.Flag.Std {
			p.error(syntax.Error{Pos: pos, Msg: fmt.Sprintf("//%s is only allowed in the standard library", verb)})
		}
		if flag == 0 && !allowedStdPragmas[verb] && gd.Flag.Std {
...
func isCgoGeneratedFile(gd *base.Invocation, pos syntax.Pos) bool {
	// We need the absolute file, independent of //line directives,
	// so we call pos.Base().Pos().
	return strings.HasPrefix(filepath.Base(trimFilename(gd, pos.Base().Pos().Base())), "_cgo_")
}
...
func checkEmbed(gd *base.Invocation, decl *syntax.VarDecl, haveEmbed, withinFunc bool) error {
	switch {
	case !haveEmbed:
	...
	case !types.AllowsGoVersion(1, 16):
		return fmt.Errorf("go:embed requires go1.16 or later (-lang was set to %s; check go.mod)", gd.Flag.Lang)
