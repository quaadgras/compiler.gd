...
func Funcs(gd *base.Invocation, fns []*ir.Func) {
	if gd.Flag.N != 0 || gd.Debug.NoDeadLocals != 0 {
		return
	}

	zero := ir.NewBasicLit(gd, gd.AutogeneratedPos, types.Types[types.TINT], constant.MakeInt64(0))

	for _, fn := range fns {

		if fn.IsClosure() {
			if ir.MatchAstDump(gd, fn, "deadlocals closure") {
				ir.AstDump(gd, fn, "deadlocals closure skipped, "+ir.FuncName(fn))
			}
			continue
		}
		...
		if ir.MatchAstDump(gd, fn, "deadlocals") {
			ir.AstDump(gd, fn, "deadLocals, "+ir.FuncName(fn))
		}
	}
}
