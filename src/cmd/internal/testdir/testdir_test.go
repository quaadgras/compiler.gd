...
var (
	...
	gdAsmCheck     = flag.Bool("gd-asmcheck", false, "run asmcheck tests under the gd fork (off by default; gd codegen diverges from stock)")
...
func (t test) run() error {
	...
	case "asmcheck":
		...
		//
		// gd fork: the codegen tests pin specific instruction
		// sequences ("expected MOVL $0, X" etc.) generated by
		// stock cmd/compile. The fork's escape-bits, fat-iface
		// 16-byte interface representation, SSO 24-byte string
		// header, and Phase G in-place rewrites legitimately
		// change generated code in many of these patterns
		// (return-by-pointer rewrites change move shapes,
		// fat-iface conversion uses OMAKEFACE inline slots
		// instead of convT calls, etc.). Skip them by default;
		// re-enable with -gd-asmcheck if you want to assert
		// individual intrinsics or ABI invariants and have
		// updated the patterns. Detected via runtime.Version()
		// because runtime.Compiler stays "gc" — gd reuses the gc
		// backend.
		if strings.HasPrefix(runtime.Version(), "gd") && !*gdAsmCheck {
			t.Skip("asmcheck patterns target stock cmd/compile output; gd codegen diverges (escape-bits, fat-iface, SSO, in-place rewrites). Use -gd-asmcheck to override.")
		}
